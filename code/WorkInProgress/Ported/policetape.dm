...
/obj/item/tape/barbwire
	density = 0
	name = "barbwire"
	desc = "A length of barbwire. Do not cross."
	icon_base = "barbwire"


/obj/item/tape/barbwire/HasEntered(atom/movable/AM as mob|obj)
	if(ismob(AM))
		var/mob/living/carbon/human/H = AM
		if (istype(AM, /mob/living/carbon/human))
			var/damage = rand(1,2)
			H.canmove = 0
			H.apply_damage(damage, BRUTE, "groin")
			H.apply_damage(damage, BRUTE, "l_leg")
			H.apply_damage(damage, BRUTE, "r_leg")
			H.apply_damage(damage, BRUTE, "l_foot")
			H.apply_damage(damage, BRUTE, "r_foot")
			sleep(30)
			H.canmove = 1

/obj/item/tape/barbwire/attack_hand(mob/user as mob)
	if (user.a_intent == "help" && src.allowed(user))
		usr << "\red No! Its sharp!."
...
/obj/item/tape/proc/breaktape(obj/item/weapon/W as obj, mob/user as mob)
	if(user.a_intent == "help" && ((!is_sharp(W) && src.allowed(user)) ||(!is_cut(W) && !src.allowed(user))))
		...
		return
	user << "\blue You start break the [src]."
	sleep(30)
