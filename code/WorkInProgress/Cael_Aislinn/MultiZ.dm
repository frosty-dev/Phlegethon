...
/turf/simulated/floor/open
	...
	New()
		...
		spawn(1)
			...
			if(floorbelow)
				...
				if(!istype(floorbelow,/turf/simulated/floor))
	...
	Enter(var/atom/movable/AM)
		...
		if(istype(AM) && (AM.pass_flags & PASSTABLE || AM.flags & TABLEPASS || AM.throwing))   //phleg. ???
			...
			spawn(1)
				if(AM)
					...
					if (istype(AM, /mob/living/carbon/human))
						...
						H.apply_damage(1*damage, BRUTE, "l_leg")
						H.apply_damage(1*damage, BRUTE, "r_leg")
...
/*
	Enter(var/atom/movable/AM)
		if (istype(AM, /mob/living/carbon/human))
			...
			H.apply_damage(0.5*damage, TOX, "l_leg")
			H.apply_damage(0.5*damage, TOX, "r_leg")
*/
	New()
		..()
		ul_SetLuminosity(0,2,0)

/turf/simulated/floor/waste/Entered(AM)
	if(ismob(AM))
		var/mob/living/carbon/human/H = AM
		if (istype(AM, /mob/living/carbon/human))
			var/damage = rand(5,10)
			...
			if(prob(20))
				...
				if(prob(98))
					...
					randmutg(H)
				domutcheck(H, null)
				updateappearance(H,H.dna.uni_identity)



/turf/simulated/floor/transit
	name = "wasteland"
	intact = 0
	icon = 'multiz.dmi'
	icon_state = "open_west"
	//floorstrength = 1
	mouse_opacity = 2
		...
		if (..()) //TODO make this check if gravity is active (future use) - Sukasa
			spawn(1)
				if (istype(AM, /mob/living/carbon/human))
					var/mob/living/carbon/human/H = AM
					var/damage = rand(5,15)
					H.apply_damage(5*damage, BRUTE, "head")
					H.apply_damage(5*damage, BRUTE, "chest")
					H.apply_damage(5*damage, BRUTE, "l_leg")
					H.apply_damage(5*damage, BRUTE, "r_leg")
					H.apply_damage(5*damage, BRUTE, "l_arm")
					H.apply_damage(5*damage, BRUTE, "r_arm")
