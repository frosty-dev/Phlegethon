	...
	reagent
		...
		proc
			...
			on_mob_life(var/mob/living/M as mob)
				...
				holder.remove_reagent(src.id, REAGENTS_METABOLISM*10) //By default it slowly disappears.
			//	M.nutrition -= REAGENTS_METABOLISM //phleg
				return
			...
			reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)
				...
				if(method == TOUCH)
					if(istype(M, /mob/living/carbon/human))
						...
						if(!M.unacidable)
							M.take_organ_damage(min(20, volume * 4)) // same deal as sulphuric acid
				else
					if(!M.unacidable)
						if(istype(M, /mob/living/carbon/human))
							var/datum/organ/external/head/affecting = M:get_organ("head")
...
// 	condiments, additives, and such go.
		nutriment
			...
			on_mob_life(var/mob/living/M as mob)
				...
				volume -= 5 * REAGENTS_METABOLISM //phleg dognail
//phleg
...
/*
		harpyment
			name = "Harpyment"
			id = "harpyment"
			description = "Its human..."
			reagent_state = SOLID
			color = "#664330" // rgb: 102, 67, 48

			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(prob(10)) M:heal_organ_damage(0.5,0) //This should stop being able to heal out of crit from eating a donut
				M:harpy += 1
...
//				confused_start = 40		//amount absorbed after which mob starts confusing directions //This is quite silly - Erthilo
	...
	//					M:adjustToxLoss(0.1)
			...
			moonshine	//gemini
				name = "Moonshine"
				id = "moonshine"
				description = "White homebrew whiskey for true rednecks."
				color = "#664300" // rgb: 102, 67, 0
				slur_start = 30
				dizzy_adj = 3
				blur_start = 80
				...
				on_mob_life(var/mob/living/M as mob)
					...
					if(volume > REAGENTS_OVERDOSE)
						M:adjustToxLoss(2)
					if (M.bodytemperature < 315)//310 is the normal bodytemp. 310.055
						M.bodytemperature = min(315, M.bodytemperature+5)
					return
...
//crc
		...
		lazarus
			name = "Lazarus"
			id = "lazarus"
			description = "A strange black liquid"
			reagent_state = LIQUID
