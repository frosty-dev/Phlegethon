					...
					if(D)
						for(var/mob/living/carbon/M in D.loc)
							...
							if (istype(M, /mob))
								...
								message_admins("ATTACK: [user] ([user.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) shot [M] ([M.ckey]) with a syringegun ([R]).")
							else
								...
								message_admins("ATTACK: UNKNOWN shot [M] ([M.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[M]\">Jump</a>) with a <b>syringegun</b> ([R]).")
...
/obj/item/weapon/gun/bow
	name = "bow"
	desc = "Let your inner huntsman go wild. Not as good as the crossbow, but it's much easier to use."
	icon = 'gun.dmi'
	icon_state = "bow"
	item_state = "bow"
	...
	examine()
		...
		..()
		if (!(usr in view(2)) && usr!=src.loc) return
		usr << "\blue [arrows.len] / [max_arrows] arrows."

	attackby(obj/item/I as obj, mob/user as mob)

		if(istype(I, /obj/item/weapon/reagent_containers/arrow))
			if(arrows.len < max_arrows)
				user.drop_item()
				I.loc = src
				arrows += I
				user << "\blue You put the arrow in [src]."
				user << "\blue [arrows.len] / [max_arrows] arrows."
			else
				usr << "\red [src] cannot hold more arrows."

	afterattack(obj/target, mob/user , flag)
		if(!isturf(target.loc) || target == user) return

		if(arrows.len)
			spawn(0) fire_arrow(target,user)
		else
			usr << "\red [src] is empty."

	proc
		fire_arrow(atom/target, mob/user)
			if (locate (/obj/structure/table, src.loc))
				return
			else
				var/turf/trg = get_turf(target)
				var/obj/effect/syringe_gun_dummy/D = new/obj/effect/syringe_gun_dummy(get_turf(src))
				var/obj/item/weapon/reagent_containers/arrow/S = arrows[1]
				if((!S) || (!S.reagents))	//ho boy! wot runtimes!
					return
				S.reagents.trans_to(D, S.reagents.total_volume)
				arrows -= S
				del(S)
				D.icon_state = "arrowproj"
				D.name = "arrow"
				playsound(user.loc, 'syringeproj.ogg', 50, 1)
					...
					if(D)
						for(var/mob/living/carbon/M in D.loc)
							...
							for(var/datum/reagent/A in D.reagents.reagent_list)
								...
								R += num2text(A.volume) + "),"
							if (istype(M, /mob))
								M.attack_log += "\[[time_stamp()]\] <b>[user]/[user.ckey]</b> shot <b>[M]/[M.ckey]</b> with a <b>bow</b> ([R])"
								user.attack_log += "\[[time_stamp()]\] <b>[user]/[user.ckey]</b> shot <b>[M]/[M.ckey]</b> with a <b>bow</b> ([R])"
								log_attack("<font color='red'>[user] ([user.ckey]) shot [M] ([M.ckey]) with a bow ([R])</font>")
								log_admin("ATTACK: [user] ([user.ckey]) shot [M] ([M.ckey]) with a bow ([R]).")
								message_admins("ATTACK: [user] ([user.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) shot [M] ([M.ckey]) with a bow ([R]).")
							else
								M.attack_log += "\[[time_stamp()]\] <b>UNKNOWN SUBJECT (No longer exists)</b> shot <b>[M]/[M.ckey]</b> with a <b>bow</b> ([R])"
								log_attack("<font color='red'>UNKNOWN shot [M] ([M.ckey]) with a <b>bow</b> ([R])</font>")
								log_admin("ATTACK: UNKNOWN shot [M] ([M.ckey]) with a <b>bow</b> ([R]).")
								message_admins("ATTACK: UNKNOWN shot [M] ([M.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[M]\">Jump</a>) with a <b>bow</b> ([R]).")
							D.reagents.trans_to(M, 5)
							M.take_organ_damage(roll("6d6"))
							for(var/mob/O in viewers(world.view, D))
								O.show_message(text("\red [] is hit by the arrow!", M.name), 1)
							new /obj/item/weapon/reagent_containers/arrow(D.loc)
							del(D)
					if(D)
						for(var/atom/A in D.loc)
							if(A == user) continue
...
/obj/item/weapon/reagent_containers/glass
	...
	afterattack(obj/target, mob/user , flag)
		...
		if(ismob(target) && target.reagents && reagents.total_volume)
			...
			for(var/mob/living/carbon/human)
				human.burninglit = 0 //phleg
			...
			if(SYRINGE_DRAW)
				...
				if(ismob(target))//Blood!
					if(istype(target, /mob/living/carbon/metroid))
						...
						return
					if(src.reagents.has_reagent("blood"))
						user << "\red There is already a blood drops on this arrow"
						return
					if(istype(target, /mob/living/carbon))//maybe just add a blood reagent to all mobs. Then you can suck them dry...With hundreds of syringes. Jolly good idea.
						var/amount = src.reagents.maximum_volume - src.reagents.total_volume
						var/mob/living/carbon/T = target
...
//						T:vessel.trans_to(src, amount) // Virus2 and antibodies aren't in blood in the first place.
						...
						user << "\blue You take a blood drops from [target]"
						for(var/mob/O in viewers(4, user))
							O.show_message("\red [user] takes a blood drops from [target].", 1)
					...
					if(!target.reagents.total_volume)
						user << "\red [target] is clear."
						return

					if(!target.is_open_container() && !istype(target,/obj/structure/reagent_dispensers) && !istype(target,/obj/item/metroid_core))
						user << "\red You cannot directly extract reagents from this object."
						return

					var/trans = target.reagents.trans_to(src, amount_per_transfer_from_this) // transfer from, transfer to - who cares?

					user << "\blue You smeared the arrow with [trans] units of the solution."
				if (reagents.total_volume >= reagents.maximum_volume)
					mode=!mode
					update_icon()
			...
			if(SYRINGE_INJECT)
				if(!reagents.total_volume)
					user << "\red The Arrow is clear."
					return
				if(istype(target, /obj/item/weapon/implantcase/chem))
					return

				if(!target.is_open_container() && !ismob(target) && !istype(target, /obj/item/weapon/reagent_containers/food) && !istype(target, /obj/item/metroid_core))
					user << "\red You cannot make it."
					return
				if(target.reagents.total_volume >= target.reagents.maximum_volume)
					user << "\red [target] is full."
					return
				...
				if(ismob(target) && target != user)
					for(var/mob/O in viewers(world.view, user))
						O.show_message(text("\red <B>[] is trying to thrust []!</B>", user, target), 1)
					if(!do_mob(user, target, 0)) return
					for(var/mob/O in viewers(world.view, user))
						O.show_message(text("\red [] stained [] with the arrow!", user, target), 1)
					src.reagents.reaction(target, INGEST)
					if(prob(2) && istype(target,/mob/living/carbon/monkey))
						var/mob/living/carbon/monkey/M = target
						M.react_to_attack(user)
				...
				spawn(5)
					...
					if(B && ishuman(target))
						...
						trans = src.reagents.trans_to(target, amount_per_transfer_from_this)
					user << "\blue You clear [trans] units of the solution. The arrow now contains [src.reagents.total_volume] units."
					if (reagents.total_volume <= 0 && mode==SYRINGE_INJECT)
						mode = SYRINGE_DRAW
						update_icon()
		...
		if(ismob(loc))
			...
			switch(mode)
				...
				if (SYRINGE_INJECT)
					injoverlay = "thrust"
			overlays += injoverlay
		icon_state = "a[rounded_vol]"
		item_state = "arrow_[rounded_vol]"
		if(reagents.total_volume)
			var/obj/effect/overlay = new/obj
			overlay.icon = 'syringefilling.dmi'
			switch(rounded_vol)
				if(5)	overlay.icon_state = "a5"
			//	if(10)	overlay.icon_state = "10"
			//	if(15)	overlay.icon_state = "15"
...
/obj/item/weapon/reagent_containers/hypospray/attack(mob/M as mob, mob/user as mob)
	...
	if (reagents.total_volume)
		...
		message_admins("ATTACK: [user] ([user.ckey](<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) injected [M] ([M.ckey]) with [src].")
		...
		if(!reagents.total_volume)
			...
			switch(trash)
				...
				if ("cheesie")
					...
					M.put_in_hand(T)
				if ("cansoup")
					var/obj/item/trash/cansoup/T = new /obj/item/trash/cansoup/( M )
					M.put_in_hand(T)
				if ("canbeans")
					var/obj/item/trash/canbeans/T = new /obj/item/trash/canbeans/( M )
					M.put_in_hand(T)
				if ("canfruit")
					var/obj/item/trash/canfruit/T = new /obj/item/trash/canfruit/( M )
					M.put_in_hand(T)
				if ("canmre")
					var/obj/item/trash/canmre/T = new /obj/item/trash/canmre/( M )
					M.put_in_hand(T)
				if ("cangator")
					var/obj/item/trash/cangator/T = new /obj/item/trash/cangator/( M )
					M.put_in_hand(T)
				if ("chips")
		...
		if(istype(M, /mob/living/carbon))
			if(M == user)								//If you're eating it yourself.
				...
				if(!istype(M, /mob/living/carbon/metroid))		//If you're feeding it to someone else.
					...
					message_admins("ATTACK: [user] ([user.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) fed [M] ([M.ckey]) with [src].")
...
/obj/item/weapon/broken_bottle	//phleg
...
/obj/item/weapon/broken_bottle/afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
...
/obj/item/weapon/reagent_containers/glass/bottle/gpowder
	...
	New()
		..()
		reagents.add_reagent("gpowder", 30)
...
/obj/item/weapon/reagent_containers/glass/bottle/insanity
	name = "Insanity culture bottle"
	desc = "A small bottle. Contains Insanity Rabies A82.11 culture in synthblood medium."//Or simply - General BullShit
	icon = 'chemical.dmi'
	icon_state = "bottle3"
	New()
		..()
		var/datum/disease/F = new /datum/disease/insanity(0)
		var/list/data = list("viruses"= list(F))
		reagents.add_reagent("blood", 20, data)
...
/obj/item/weapon/reagent_containers/food/drinks/lazarus
	name = "Lazarus vial"
	desc = "Strange prehistoric vial."
	icon_state = "vial"
	New()
		..()
		reagents.add_reagent("lazarus", 15)
		src.pixel_x = rand(-10.0, 10)
		src.pixel_y = rand(-10.0, 10)
...
//		var/headarmor = 0 // Target's head armour
		armor_block = H.run_armor_check("melee") // For normal attack damage

		//If they have a hat/helmet and the user is targeting their head.
		if(istype(H.head, /obj/item/clothing/head) && user.zone_sel.selecting == "head")

			// If their head has an armour value, assign headarmor to it, else give it 0.
			if(H.head.armor["melee"])
				H.head.armor = H.head.armor["melee"]
			else
				H.head.armor = 0
		else
			H.head.armor = 0
...
//		armor_duration = (duration - headarmor) + force
		...
		armor_block = target.run_armor_check("melee")
		if(user.zone_sel.selecting == "head")
			target.apply_effect(min(5) , WEAKEN)
	//		armor_duration = duration + force
//	armor_duration /= 10

	//Apply the damage!
	target.apply_damage(force, BRUTE, armor_block)

	// You are going to knock someone out for longer if they are not wearing a helmet.
	if(user.zone_sel.selecting == "head" && istype(target, /mob/living/carbon/))
...
*/
/*
/obj/item/weapon/reagent_containers/food/drinks/bottle/attack(mob/M as mob, mob/user as mob)
	if (user.a_intent == "harm")
		playsound(src.loc, 'Glassbr3.ogg', 60, 1)
		user.visible_message("\red <B>\The [user] smashed a bottle!</B>")
...
/obj/item/weapon/reagent_containers/food/drinks/bottle/moonshine
	name = "Moonshine"
	desc = "White homebrew whiskey for true rednecks."
	...
	on_reagent_change()
		...
		if (reagents.reagent_list.len > 0)
			...
			switch(reagents.get_master_reagent_id())
				...
				if("lazarus")
					icon_state = "lazarus_glass"
					name = "lazarus"
					desc = "A Strange black liquid."
				if("iced_beer")
