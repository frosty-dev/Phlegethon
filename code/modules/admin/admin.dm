		...
		switch(href_list["simplemake"])
			...
			if("meme")
				...
				if(delmob)
					del(M)
			if("zombie") //phleg
				var/mob/living/parasite/zombie/newzombie = new
				M.mind.transfer_to(newzombie)
				newzombie.clearHUD()

				var/found = 0
				for(var/mob/living/carbon/human/H in world) if(H.client && !H.parasites.len)
					found = 1
					newzombie.enter_host(H)

					message_admins("[H] has become [newzombie.key]'s host")

					break

				// if there was no host, abort
				if(!found)
					newzombie.mind.transfer_to(M)
					message_admins("Failed to find host for zombie [M.key]. Aborting.")

				ticker.mode.zombies += newzombie
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["MetropolisReply"])
		var/mob/M = locate(href_list["MetropolisReply"])
		if(!M)
			return
		if(!istype(M, /mob/living/carbon/human))
			alert("Metropolis cannot transmit to non-humans.")
			return
		if((!istype(M:l_ear, /obj/item/device/radio/headset)) && (!istype(M:r_ear, /obj/item/device/radio/headset)))
			alert("The person you're trying to reply to doesn't have a headset!  Metropolis cannot transmit directly to them.")
			return
		var/input = input(src.owner, "Please enter a message to reply to [key_name(M)] via their headset.","Outgoing message from Metropolis", "")
		if(!input)
			return

		src.owner << "You sent [input] to [M] via a secure channel."
		log_admin("[src.owner] replied to [key_name(M)]'s Metropolis message with the message [input].")
	...
	//knock yourself out to stop the ghosts
				...
				if("virus")
					...
					if(answer=="Random")
						...
						var/list/viruses = list("fake gbs","gbs","insanity","magnitis","wizarditis",/*"beesease",*/"brain rot","cold","retrovirus","flu","pierrot's throat","rhumba beat")
...
/obj/admins/proc/show_player_panel(var/mob/M in world)
	...
	if (M.client)
		if(!istype(M, /mob/new_player))
			...
			body += "<A href='?src=\ref[src];simplemake=zombie;mob=\ref[M]'>Zombie</A>"
			body += "<br>"
	...
	if(!show_harpy)
		...
		for(var/mob/M in mobs)
			if(M.ckey)
				...
				var/DBQuery/query = dbcon.NewQuery("SELECT harpy FROM harpytotals WHERE byondkey='[M.key]'")
				query.Execute()
