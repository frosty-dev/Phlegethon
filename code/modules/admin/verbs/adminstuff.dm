...
//	dat += "<A HREF='?src=\ref[src];controlpanel=editfile'>Edit textfile</A><br>"
	...
	dat += "Harpyness: <A HREF='?src=\ref[src];controlpanel=sql_harpyspender'>show changes by player</A><br>"
	dat += "Harpyness: <A HREF='?src=\ref[src];controlpanel=sql_harpyreceiver'>show player harpy changes</A><br>"
...
//phleg
			if("sql_harpyspender")
				var/spender = input("Input spender key:","Show harpy changes") as text|null
				if(!spender)
					return
				var/DBConnection/dbcon = new()
				dbcon.Connect("dbi:mysql:[sqldb]:[sqladdress]:[sqlport]","[sqllogin]","[sqlpass]")
				if(dbcon.IsConnected())
					var/DBQuery/query = dbcon.NewQuery("SELECT spendername, receiverkey, receivername, receiverrole, receiverspecial, isnegative, time FROM harpy WHERE spenderkey = '[spender]'")
					if(query.Execute())
						dat += "Harpyness spends by [spender]:<br>"
						dat += "<table border=1 cellspacing=0><tr><td>SName</td><td>RKey</td><td>RName</td><td>RRole</td><td><RSpecial></td><td></td><td>Time</td></tr>"
						while(query.NextRow())
							dat += "<tr><td>[query.item[1]]</td><td>[query.item[2]]</td><td>[query.item[3]]</td><td>[query.item[4]]</td><td>[query.item[5]]</td><td>[text2num(query.item[6]) ? "-" : "+"]</td><td>[query.item[7]]</td></tr>"
						dat += "</table>R: Receiver, S: Spender<br>"
			...
			if("sql_harpyreceiver")
				var/receiver = input("Input receiver key:","Show harpy changes") as text|null
				if(!receiver)
					return
				var/DBConnection/dbcon = new()
				dbcon.Connect("dbi:mysql:[sqldb]:[sqladdress]:[sqlport]","[sqllogin]","[sqlpass]")
				if(dbcon.IsConnected())
					var/DBQuery/query = dbcon.NewQuery("SELECT spendername, spenderkey, receivername, receiverrole, receiverspecial, isnegative, time FROM harpy WHERE receiverkey = '[receiver]'")
					if(query.Execute())
						dat += "[receiver] harpy changes:<br>"
						dat += "<table border=1 cellspacing=0><tr><td>SName</td><td>SKey</td><td>RName</td><td>RRole</td><td><RSpecial></td><td></td><td>Time</td></tr>"
						while(query.NextRow())
							dat += "<tr><td>[query.item[1]]</td><td>[query.item[2]]</td><td>[query.item[3]]</td><td>[query.item[4]]</td><td>[query.item[5]]</td><td>[text2num(query.item[6]) ? "-" : "+"]</td><td>[query.item[7]]</td></tr>"
						dat += "</table>R: Receiver, S: Spender<br>"
