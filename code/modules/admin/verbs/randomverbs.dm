...
/client/proc/respawn_character()
	...
	if(!record_found)//We have to pick their role if they have no record.
		if(G_found.mind&&G_found.mind.assigned_role)//But they may have an assigned role already.
			...
			if(!assigned_role)	new_character.mind.assigned_role = "Convict"//Defaults to assistant.
			else	new_character.mind.assigned_role = assigned_role

	if(!new_character.mind.assigned_role)	new_character.mind.assigned_role = "Convict"//If they somehow got a null assigned role.
...
/client/proc/cmd_admin_create_centcom_report()
	...
	switch(alert("Should this be announced to the general population?",,"Yes","No"))
		...
		if("No")
			world << "\red New Metropolis Update available at all communication consoles."
...
/client/proc/admin_call_shuttle()
	...
	captain_announce("The transport start fueling. [round(emergency_shuttle.timeleft()/60)] minutes left.")
...
/client/proc/toggle_gravity_on()
	...
	command_alert("Metropolis is now beaming gravitons to your polygon.  We appoligize for any inconvience.")
...
/client/proc/toggle_gravity_off()
	...
	command_alert("For budget reasons, Metropolis is no longer beaming gravitons to your polygon.  We appoligize for any inconvience.")
