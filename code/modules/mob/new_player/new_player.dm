...
/mob/new_player
	...
	proc/AnnounceArrival(var/mob/living/carbon/human/character, var/rank)
		if (ticker.current_state == GAME_STATE_PLAYING)
			...
			a.autosay("\"[character.real_name],[character.wear_id.assignment ? " [character.wear_id.assignment]," : "" ] has been sent through the the Pipes.\"", "Arrivals Announcement Computer")
	...
	proc/LateChoices()
		...
		if (respawndelaystart > 0)
			...
			var/deathtime = world.time - src.timeofdeath  //respawndelay
		...
		if(emergency_shuttle) //In case Nanotrasen decides reposess CentComm's shuttles.
			...
			if(emergency_shuttle.direction == 1 && emergency_shuttle.timeleft() < 30) //Shuttle is past the point of no recall
