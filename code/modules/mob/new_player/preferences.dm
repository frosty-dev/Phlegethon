...
//some autodetection here.
	...
	"zombie" = IS_MODE_COMPILED("zombie"),               // 11
)
...
//proc for making sentences Do This As An Example. For names.
	...
	proc/SetSkills(mob/user)
		...
		var/HTML = "<body bgcolor=\"#2C3D29\", text = \"#A5AB54\", link=\"#3B946A\" vlink=\"#3B946A\" alink=\"#2DB9B9\">"   //bgcolor=\"#2C3D29\"
		...
		for(var/V in SKILLS)
			...
			for(var/datum/skill/S in SKILLS[V])
				...
				HTML += "<th><a href='byond://?src=\ref[user];preferences=1;skills=1;setskill=\ref[S];newvalue=[SKILL_NONE]'><font color=[(level == SKILL_NONE) ? "red" : "#557050"]>\[Untrained\]</font></a></th>"
				// secondary skills don't have an amateur level
				if(S.secondary)
					HTML += "<th></th>"
				else
					HTML += "<th><a href='byond://?src=\ref[user];preferences=1;skills=1;setskill=\ref[S];newvalue=[SKILL_BASIC]'><font color=[(level == SKILL_BASIC) ? "red" : "#557050"]>\[Amateur\]</font></a></th>"
				HTML += "<th><a href='byond://?src=\ref[user];preferences=1;skills=1;setskill=\ref[S];newvalue=[SKILL_ADEPT]'><font color=[(level == SKILL_ADEPT) ? "red" : "#557050"]>\[Trained\]</font></a></th>"
				HTML += "<th><a href='byond://?src=\ref[user];preferences=1;skills=1;setskill=\ref[S];newvalue=[SKILL_EXPERT]'><font color=[(level == SKILL_EXPERT) ? "red" : "#557050"]>\[Professional\]</font></a></th>"
				HTML += "</tr>"
		HTML += "</table>"
	...
	proc/ShowChoices(mob/user)
		...
		var/dat = "<html><body bgcolor=\"#2C3D29\", text = \"#A5AB54\", link=\"#3B946A\" vlink=\"#3B946A\" alink=\"#2DB9B9\">" // befory only body
		dat += "<b>Name:</b> "
...
//		dat += "Species: <a href='byond://?src=\ref[user];preferences=1;species=input'>[species]</a><br>"
	...
	proc/loadsave(mob/user)
		var/dat = "<body bgcolor=\"#2C3D29\", text = \"#A5AB54\", link=\"#3B946A\" vlink=\"#3B946A\" alink=\"#2DB9B9\">"
		dat += "<tt><center>"
	...
	proc/SetDisabilities(mob/user)
		var/HTML = "<body bgcolor=\"#2C3D29\", text = \"#A5AB54\", link=\"#3B946A\" vlink=\"#3B946A\" alink=\"#2DB9B9\">"
		HTML += "<tt><center>"
	...
	proc/SetChoices(mob/user, limit = 17, list/splitJobs, width = 550, height = 500)
		...
		for(var/datum/job/job in job_master.occupations)
			...
			if(jobban_isbanned(user, rank))
				...
				continue
			if((job_TH_low & CONVICT) && (rank != "Convict"))
			...
			if(rank == "Convict")//Assistant is special
				if(job_TH_low & CONVICT)
	...
	proc/SetJobDepartment(var/datum/job/job, var/level)
		...
		switch(level)
			...
			if(2)//Set current highs to med, then reset them
				job_TH_med |= job_TH_high
				job_MMC_med |= job_MMC_high
				job_TH_high = 0
				job_MMC_high = 0
...
//				switch(level)
//					if(2)
//						job_MMC_high = job.flag
//						job_MMC_med &= ~job.flag
//					if(3)
//						job_MMC_med |= job.flag
//						job_MMC_low &= ~job.flag
//					else
//						job_MMC_low |= job.flag
		return 1
...
*/
		...
		if(link_tags["harpyness"])
			harpy = "harpyness [harpy]"
