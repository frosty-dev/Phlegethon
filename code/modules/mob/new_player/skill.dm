...
proc/show_skill_window(var/mob/user, var/mob/living/carbon/human/M)
	...
	for(var/V in SKILLS)
		...
		for(var/datum/skill/S in SKILLS[V])
			...
			HTML += "<th>[S.name]</th>"
			HTML += "<th><font color=[(level == SKILL_NONE) ? "red" : "#557050"]>\[Untrained\]</font></th>"
			// secondary skills don't have an amateur level
			if(S.secondary)
				HTML += "<th></th>"
			else
				HTML += "<th><font color=[(level == SKILL_BASIC) ? "red" : "#557050"]>\[Amateur\]</font></th>"
			HTML += "<th><font color=[(level == SKILL_ADEPT) ? "red" : "#557050"]>\[Trained\]</font></th>"
			HTML += "<th><font color=[(level == SKILL_EXPERT) ? "red" : "#557050"]>\[Professional\]</font></th>"
			HTML += "</tr>"
	HTML += "</table>"
