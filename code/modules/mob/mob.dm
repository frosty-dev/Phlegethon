...
/mob/proc/saveharpy_path(mob/user)
	return "data/player_saves/[copytext(user.ckey, 1, 2)]/[user.ckey]/harpy.sav"

/mob/proc/saveharpy_save(mob/user)
	if (IsGuestKey(user.key))
		return 0

	var/savefile/F = new /savefile(src.saveharpy_path(user))
...
/mob/proc/saveharpy_load(mob/user) //var/silent = 1)
	if (IsGuestKey(user.key))
		return 0

	var/path = saveharpy_path(user)
	if (!fexists(path))
		return 0
...
/mob/proc/update_canmove()
	if(buckled)
		...
		if( istype(buckled,/obj/structure/stool/bed/chair) )
			...
			lying = 1
	else if( stat || weakened || paralysis || resting || sleeping )
		lying = 1
		canmove = 0
	else if( stunned )
//		lying = 0
...
/mob/proc/harpypoint()
	if (harpy >= 15)
		captain_announce("The [src:real_name] get 15 harpyness points!.")
	if (harpy >= 30)
		captain_announce("The [src:real_name] get 30 harpyness points!.")
	if (harpy >= 50)
		captain_announce("The [src:real_name] get 50 harpyness points!.")
	if (harpy >= 100)
		captain_announce("The [src:real_name] get 100 harpyness points!.")
	if (harpy >= 150)
		captain_announce("The [src:real_name] get 150 harpyness points!.")
	if (harpy >= 200)
		captain_announce("The [src:real_name] get 200 harpyness points!.")
	return 0


/mob/proc/harpiness()
	if(lastattacked != null)
		lastattacker.harpy += 1
