...
var/global/const/MAXIMUM_ZOMBIE_POINTS = 750
...
mob/living/carbon/list/parasites = list()
...
mob/living/parasite/zombie/var/zombie_points = 100
mob/living/parasite/zombie/var/dormant = 0

// Memes have a list of indoctrinated hosts
mob/living/parasite/zombie/var/list/indoctrinated = list()

mob/living/parasite/zombie/Life()
	..()
	...
	zombie_points = min(zombie_points + gain, MAXIMUM_ZOMBIE_POINTS)
	...
	if(host.stat == 2)
		...
		spawn
			...
			var/datum/mind/zombie_mind = src.mind

			host_mind.transfer_to(dummy)
			zombie_mind.transfer_to(host)
			host_mind.current.clearHUD()
			host.update_clothing()

			dummy << "\blue You feel very drowsy.. Your eyelids become heavy..."

			log_admin("[zombie_mind.key] has taken possession of [host]([host_mind.key])")
			message_admins("[zombie_mind.key] has taken possession of [host]([host_mind.key])")
//		src.death()
		return
...
mob/living/parasite/zombie/death()
...
mob/living/parasite/zombie/say(message as text)
	if(dormant)
...
mob/living/parasite/zombie/whisper(message as text)
	if(dormant)
		usr << "\red You're dormant!"
		return
	if(!host)
...
mob/living/parasite/zombie/me_verb(message as text)
	set name = "Me"
...
mob/living/parasite/zombie/say_understands(mob/other)
	if(!host) return 0

	return host.say_understands(other)

// Try to use amount points, return 1 if successful
mob/living/parasite/zombie/proc/use_points(amount)
	if(dormant)
		usr << "\red You're dormant!"
		return
	if(src.zombie_points < amount)
		src << "<b>* You don't have enough zombie points(need [amount]).</b>"
		return 0

	src.zombie_points -= round(amount)
	return 1

// Let the meme choose one of his indoctrinated mobs as target
mob/living/parasite/zombie/proc/select_indoctrinated(var/title, var/message)
	var/list/candidates
...
// Mutes the host
mob/living/parasite/zombie/verb/Mute()
	set category = "Zombie"
	set name	 = "Mute(250)"
...
mob/living/parasite/zombie/verb/Paralyze()
	set category = "Zombie"
...
mob/living/parasite/zombie/verb/Agony()
	set category = "Zombie"
	set name	 = "Agony(200)"
...
mob/living/parasite/zombie/verb/Joy()
	set category = "Zombie"
	set name	 = "Joy(200)"
...
mob/living/parasite/zombie/verb/Hallucinate()
	set category = "Zombie"
...
mob/living/parasite/zombie/verb/SubtleJump(mob/living/carbon/human/target as mob in world)
	set category = "Zombie"
...
mob/living/parasite/zombie/verb/ObviousJump(mob/living/carbon/human/target as mob in world)
	set category = "Zombie"
...
mob/living/parasite/zombie/verb/AttunedJump(mob/living/carbon/human/target as mob in world)
	set category = "Zombie"
...
mob/living/parasite/zombie/verb/Attune()
	set category = "Zombie"
	set name	 = "Infect(100)"
	set desc     = "Activating parasitic substance in host biosystem."

	if(host in src.indoctrinated)
		usr << "<b>You have already infected this host.</b>"
		return
	...
	usr << "<b>You successfully infect [host]."
	host << "\red Your head feels a bit weak.."

	log_admin("[src.key] has infected [host]")
	message_admins("[src.key] has infected [host]")

// Enables the mob to take a lot more damage
mob/living/parasite/zombie/verb/Analgesic()
	set category = "Zombie"
...
mob/living/parasite/zombie/verb/Possession()
	set category = "Zombie"
	...
	spawn
		...
		log_admin("[zombie_mind.key] has lost possession of [host]([host_mind.key])")
		message_admins("[zombie_mind.key] has lost possession of [host]([host_mind.key])")

		zombie_mind.transfer_to(src)
		host_mind.transfer_to(host)
		zombie_mind.current.clearHUD()
		host.update_clothing()
...
mob/living/parasite/zombie/verb/Dormant()
	set category = "Zombie"
	...
	while(zombie_points < MAXIMUM_ZOMBIE_POINTS)
		sleep(10)
...
mob/living/parasite/zombie/verb/Show_Points()
	set category = "Zombie"

	usr << "<b>Zombie Points: [src.zombie_points]/[MAXIMUM_ZOMBIE_POINTS]</b>"

// Stat panel to show meme points, copypasted from alien
/mob/living/parasite/zombie/Stat()
	..()
	...
	if (client && client.statpanel == "Status")
		stat(null, "Zombie Points: [src.zombie_points]")
