...
/mob/living/carbon/human/drip(var/amt as num)
	...
	var/amm = 0.3 * amt   //phleg  0.1 to 0.2
...
/mob/living/carbon/human/movement_delay()
	...
	if (bodytemperature < 283.222)
		...
		if(mutantrace == "lizard") //Soghun are more affected by the cold
			tally += 1
		if(mutantrace == "zombie") //Zombie are more affected by the cold
			tally += 1
...
//	handle_regular_fov_updates() //phlegfov
	...
	return
/*
/mob/living/carbon/human/proc/handle_regular_fov_updates()

	if(fovoverlay.overlays)
		fovoverlay.overlays = list()

	for(var/image/hud in client.images)
		if(copytext(hud.icon_state,1,4) == "hud") //ugly, but icon comparison is worse, I believe (hud.icon_state,1,4)
			client.images -= hud//del(hud)

	if (stat != 2)
		var/image/I
		if(istype(head, /obj/item/clothing/head/helmet))
			I = image("icon" = 'icons/mob/screen_full.dmi', "icon_state" = "fov_h", dir = src.dir)
			fovoverlay.overlays += I
			for(var/mob/living/M in viewers(src, null))
				if(dir==SOUTH)
					if(M.y > src.y)
						M.invisibility = 2
					//	M.overlays += image("icon" = 'icons/mob/mob.dmi', "icon_state" = "noise", layer = 18)
					else
						M.invisibility = 0
					//	M.overlays -= "noise"
				else if(dir==NORTH)
			...
			I = image("icon" = 'icons/mob/screen_full.dmi', "icon_state" = "fov", dir = src.dir)
			fovoverlay.overlays += I
			for(var/mob/living/M in viewers(src, null))
				if(dir==SOUTH)
					if((M.y - src.y) > abs(M.x - src.x))
						M.invisibility = 2
					else
						M.invisibility = 0
				else if(dir==NORTH)
					if((src.y - M.y) > abs(M.x - src.x))
						M.invisibility = 2
					else
						M.invisibility = 0
				else if(dir==EAST)
					if((src.x - M.x) > abs(M.y - src.y))
						M.invisibility = 2
					else
						M.invisibility = 0
				else if(dir==WEST)
					if((M.x - src.x) > abs(M.y - src.y))
						M.invisibility = 2
					else
						M.invisibility = 0
*/
...
/mob/living/carbon/human/proc/rebuild_clothing_overlays()
	...
	//	if(w_uniform)//I should really not need these
		w_uniform.screen_loc = ui_iclothing //phleg
		if(istype(w_uniform, /obj/item/clothing/under))
			var/t1 = w_uniform.fbyond_color
			if (!t1)
				t1 = icon_state
...
/*
	...
	if (burninglit)
		if(1)
			stand_icon.Blend(new /icon('fire.dmi', "icon_state" = "moblit", "layer" = SHIELD_LAYER), ICON_MULTIPLY)
			lying_icon.Blend(new /icon('fire.dmi', "icon_state" = "moblit", "layer" = SHIELD_LAYER), ICON_MULTIPLY)
		if(0)
			return

	if (burninglit)
		stand_icon.Blend(new /icon('fire.dmi', "icon_state" = "moblit", "layer" = SHIELD_LAYER), ICON_OVERLAY)
		lying_icon.Blend(new /icon('fire.dmi', "icon_state" = "moblit", "layer" = SHIELD_LAYER), ICON_OVERLAY)
	else
