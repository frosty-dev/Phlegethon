...
//		handle_regular_fov_updates() //phleg
...
/mob/living/carbon/human
	proc
		...
		harpy_mut()
			if (harpy >= 50)
				if(harpymut < 1)
					flavor_text = "Looks bloodthirsty."
			...
			if (harpy >= 100)
				...
				if(harpymut <= 1)
					mutantrace = "canis"
					flavor_text = "Hungry eyes and trembling hands."
				harpymut = 2
			...
			if (harpy >= 150)
				...
				if(harpymut <= 2)
					mutantrace = "harpy"
					flavor_text = "Its Harpy."
					underwear = 0
					if(!REFLEXES in augmentations)
						augmentations.Add(REFLEXES)
				...
				switch(radiation)
					if(1 to 49)
						...
						if(prob(25))
							damage = 1
							adjustToxLoss(1)
							src << "\red You feel tired and odd."
							updatehealth()
					...
					if(75 to 100)
						...
						damage = 3
						if(prob(1))
							src << "\red You feel very odd and something is wrong!"
							randmutb(src)
							domutcheck(src,null)
							emote("gasp")
							updatehealth()
...
//				canmove = 1
			...
			if(buckled)
				...
				if( istype(buckled,/obj/structure/stool/bed/chair) )
					lying = 0
				if( istype(buckled,/obj/structure/stool/bed/chair/motorcycle) )
					lying = 0
					canmove = 1
				else
					lying = 1
			else if( stat || sleeping || paralysis || stunned || weakened || resting || buckled || (changeling && changeling.changeling_fakedeath))
				lying = 1
				canmove = 0
			else if( stunned )
		//		lying = 0
		...
		//	if(reagents && stat != 2) reagents.remove_any(REAGENTS_METABOLISM)
			if(vessel && stat != 2) vessel.metabolize(src)
		//	if(prob(10))
		//		reagents.clear_reagents(src)
...
*/
			...
			if (nutrition > 0 && stat != 2)
				...
				if(stat == 1)
					...
					if(mutantrace == "zombie") //Zombies get hungry much faster
						nutrition = max (0, nutrition - HUNGER_FACTOR*1.5)
					else
		...
		handle_organs()
			...
			for(var/name in organs)
				...
				if(E.status & ORGAN_BROKEN || E.status & ORGAN_DESTROYED)
					if(E.name == "l_hand" || E.name == "l_arm")
						if(hand && equipped())
							if(E.status & ORGAN_SPLINTED && prob(5))
								drop_item()
								emote("scream")
							else
								drop_item()
				...
				var/datum/organ/external/e = src.organs[name]
				...
				if(health >=100)
					...
					if(e.status & ORGAN_BROKEN)
						if(prob(10))
							...
							update_body()
				//ballistic phleg
	//			if(e.btrauma > 0)	to rapid
	//				if(prob(e.btrauma*2))
	//					e.bacid += 1

				if(e.bacid > 0)
					if(prob(e.bacid*2))
						adjustToxLoss(e.bacid * 2)
						e.bacid += 1
						eye_blurry = max(e.bacid, eye_blurry)
						stuttering = max(stuttering, e.bacid * 2)
						src << "\red You feel really bad pain in [e.name] from ballistic wound"
...
//phleg
		burninglit()
			if (burninglit == 1)
				clothing_overlays += image("icon" = 'fire.dmi', "icon_state" = "moblit", "layer" = SHIELD_LAYER)
				var/damage = roll("1d10")
...
//phlegthroatcutting
		...
		handle_regular_status_updates()
			...
			throat_check()	//phlegthroatcutting
			burninglit()
	//		hiding()
			...
			if(head && !head.disfigured)
				if(head.brute_dam >= 60 || head.burn_dam >= 60)
					emote("scream")
					disfigure_face()
					face_op_stage = 0.0
...
//phlegtg
			...
			if (stat == 2 || (XRAY in mutations))
				...
				sight &= ~SEE_MOBS
				sight &= ~SEE_OBJS
				if (mutantrace == "lizard" || mutantrace == "metroid" || mutantrace == "zombie")
					see_in_dark = 3
					see_invisible = 1
		...
		// The purpose of this is to stop xray and w/e from preventing you from using huds -- Love, Doohl
			...
			// Update the nutrition icon   //phleg regen
			// -------------------------
			if (nutrition_icon)
				switch(nutrition)
					if(400 to INFINITY)
