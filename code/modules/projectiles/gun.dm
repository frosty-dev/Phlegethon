/obj/item/weapon/gun
	...
	var/condition = 0 //phleg
	var/blowback = 0//phleg
	var/armed = 0
	var/revolver = 0
	var/singleaction = 0
	var/acc = 0 //lesser = better
	...
	proc/special_check(var/mob/M) //Placeholder for any special checks, like detective's revolver.
		if(ishuman(M))
			...
			if(HM.mutantrace != "zombie")
				return 1
			M << "\red You cant use it!"
		return 0
		return 1
		 ...
		 || istype(src.in_chamber, /obj/item/projectile/bullet)) && !istype(in_chamber,/obj/item/projectile/energy/electrode)) //Point blank shooting.
			...
			if(silenced)
				if(istype(src,/obj/item/weapon/gun/projectile/shotgun))
					playsound(user, 'silencedshotgun.ogg', 30, 1)
				else
					playsound(user, 'silencedgun.ogg', 30, 1)

			else
				playsound(user, fire_sound, 50) //,1)  //point blanc phleg . 30 to 40
			M.apply_damage(40+in_chamber.damage, BRUTE, user.zone_sel.selecting, used_weapon = "Point Blank Shot") //So we'll put him an inch from death.
			...
			message_admins("ATTACK: [user] ([user.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) shot [M] ([M.ckey]) point blank with [src].")
			del(in_chamber)
			update_icon()
			return
			...
			message_admins("ATTACK: [user] ([user.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) stunned [M] ([M.ckey]) with [src].")
			del(in_chamber)
			update_icon()
			return
...
//HE MOVED, SHOOT HIM!
	...
	afterattack(atom/A as mob|obj|turf|area, mob/living/user as mob|obj, flag, params)
		...
		if(user.a_intent == "hurt" && user:gloves && istype(user:gloves,/obj/item/clothing/gloves/glinger))
			user:swap_hand()
