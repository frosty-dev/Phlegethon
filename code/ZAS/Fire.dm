...
turf/simulated/hotspot_expose(exposed_temperature, exposed_volume, soh)
  ...
  /*if(active_hotspot)
	...
	if(air_contents.calculate_firelevel(liquid) > vsc.IgnitionLevel && (fuel || liquid || air_contents.toxins > 0.5))
		igniting = 1
		if (prob(10))//(air_contents.oxygen < 0.5)	phleg
			return 0
	...
	fire
		...
		process()
			...
			if(firelevel > vsc.IgnitionLevel)
				...
				//				var
				//phleg				datum/gas_mixture/tair_contents = enemy_tile.return_air()
									//Get whatever trace fuels are in the area
				//phleg				datum/gas/volatile_fuel/tfuel = locate(/datum/gas/volatile_fuel/) in tair_contents.trace_gases
									//Also get liquid fuels on the ground.
				//phleg				obj/liquid_fuel/tliquid = locate() in enemy_tile

								if (prob(30))//(tair_contents.oxygen < 0.5 || !(tair_contents.toxins || tfuel || tliquid)) phleg
					...
					if(flow)
						...
						if(prob(75))//(flow.oxygen > 0.3 && (flow.toxins || fuel || liquid)) phleg
			...
			for(var/mob/living/carbon/human/M in loc)
				M.FireBurn(min(max(0.5,firelevel / 20),10)) //Burn the humans!  phleg 0.1
