...
#define SHUTTLEARRIVETIME 300		// 15 minutes = 900 seconds
#define SHUTTLELEAVETIME 300		// 5 minutes = 300 seconds
#define SHUTTLETRANSITTIME 60		// 3 minutes = 180 seconds
...
datum/shuttle_controller
	...
	var/departed = 0
	var/sound_siren = 1 //if siren should go off  //People hat it :(
	// call the shuttle
		...
		if(direction == 1)
			...
			if(alert == 0)
				if(timeleft >= SHUTTLEARRIVETIME)
					return
				captain_announce("The transport interrupted the refueling.")
			...
			switch(location)
				if(0)
					...
					/* --- Shuttle has docked with the station - begin countdown to transit --- */
						...
						if(alert == 0)
							captain_announce("The Transport in warm-up engine phase. You have [timeleft()/60] minutes to board the Transport.")
				...
				if(1)
					...
					/* --- Shuttle leaves the station, enters transit --- */
						...
						*/
						...
						if(alert == 0)
							captain_announce("The Transport has left the polygon. Estimate [timeleft()/60] minutes until the transport arrive at Metropolis.")
						else
							captain_announce("The transport has left the polygon. Estimate [timeleft()/60] minutes until the transport arrive at Metropolis.")
