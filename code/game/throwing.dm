...
//	if (istype(item, /obj/item/weapon/grab))  //phleg mob throwing
	...
	if(istype(hit_atom,/mob/living))
		...
		if(!istype(src, /obj/item)) // this is a big item that's being thrown at them~
			...
			//	var/armor_block = M:run_armor_check("chest", "melee")
				M:apply_damage(roll("3d6"), BRUTE, "chest")//, armor_block)

				visible_message("\red <B>[M] has been knocked down by the force of [src]!</B>")
				M:apply_effect(roll("2d6"), WEAKEN)//, armor_block)

				M:UpdateDamageIcon()
			else
				M.take_organ_damage(roll("2d6"))
			...
			message_admins("ATTACK: [hit_atom] ([M.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[M]\">Jump</a>) was hit by [src] thrown by ([src.fingerprintslast])")
