...
/obj/machinery/turret/proc/setState(var/on, var/lethal)
	src.on = on
	src.lasers = lethal
	src.power_change()
...
//	if(!enabled || !protected_area || protected_area.turretTargets.len<=0)
	if(!on || !protected_area || protected_area.turretTargets.len<=0)
		if(!isDown() && !isPopping())
			popDown()
		return
...
//	while(src && enabled && !stat && check_target(cur_target))
	while(src && on && !stat && check_target(cur_target))
...
/obj/machinery/turretid/attack_hand(mob/user as mob)
	...
	if(src.locked && (!istype(user, /mob/living/silicon)))
		...
		t += text("Turrets [] - <A href='?src=\ref[];toggleOn=1'>[]?</a><br>\n", src.on?"activated":"deactivated", src, src.on?"Disable":"Enable")
...
/obj/machinery/turret/gun_turret
	name = "Gun Turret"
	density = 0
	anchored = 1
	var/cooldown = 20
	...
	var/assigned_role = list("Chief Engineer", "Overseer", "Captain", "Research Director", "Chef")
	var/scan_range = 7
	health = 500
	popping = 0
	wasvalid = 0
	lastfired = 0
	var/list/scan_for = list("human"=1,"cyborg"=0,"mecha"=0,"alien"=1)
	on = 0
	raised = 0
...
*/
	...
	attack_hand(mob/user as mob)
		user.machine = src
		if(src.locked && (!istype(user, /mob/living/silicon)))
			usr << "\blue Swipe ID card to unlock control panel"
		else
			var/dat = {"<html>
...
//phleg
	...
	proc/get_target()
		...
		for(var/mob/living/carbon/human/M in oview(scan_range,src))
			if (istype(M:wear_id, /obj/item/weapon/card/id))
	//		if(M.mind && M.mind.assigned_role)
	//			M.mind.assigned_role = assigned_role
		//	if(M.wear_id)
				return
			if(istype(M:wear_id, null))
				continue
	...
	//	while(src && enabled && !stat && check_target(cur_target))
		...
		return
/*
	shootAt(var/atom/movable/target)
		var/turf/T = get_turf(src)
		var/turf/U = get_turf(target)
		if (!T || !U)
			return
...
*/
	...
	attackby(obj/item/weapon/W, mob/user)//I can't believe no one added this before/N
		..()
		playsound(src.loc, 'smash.ogg', 60, 1)
		src.spark_system.start()
		src.health -= W.force * 0.5
