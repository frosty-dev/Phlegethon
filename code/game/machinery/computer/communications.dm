...
/obj/machinery/computer/communications/Topic(href, href_list)
	...
	switch(href_list["operation"])
		...
		if("MessageMetropolis")
			if(src.authenticated==2)
				if(centcomm_message_cooldown)
					usr << "Arrays recycling.  Please stand by."
					return
				var/input = copytext(sanitize(input(usr, "Please choose a message to transmit to Metropolis via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response.", "To abort, send an empty message.", "")),1,MAX_MESSAGE_LEN)
				if(!input || !(usr in view(1,src)))
					return
				Centcomm_announce(input, usr)
				usr << "Message transmitted."
				log_say("[key_name(usr)] has sent Metropolis a message: [input]")
		...
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				if (src.authenticated==2)
					...
					if(src.emagged == 0)
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=MessageMetropolis'>Send an emergency message to Metropolis</A> \]"
					else
				...
				if(emergency_shuttle.location==0)
					if (emergency_shuttle.online)
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=cancelshuttle'>Cancel Transport fueling</A> \]"
					else
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=callshuttle'>Transport fueling</A> \]"
...
/mob/living/silicon/ai/proc/ai_call_shuttle()
	...
	var/confirm = alert("Are you sure you want to fueling transport?", "Confirm Transport fueling", "Yes", "No")
...
/proc/call_shuttle_proc(var/mob/user)
	...
	if(emergency_shuttle.deny_shuttle)
		user << "No fuel available in your sector. Please try again later."
		return

	if(sent_strike_team == 1)
		user << "Metropolis will not allow the fueled transport. Consider all contracts terminated."
		return

	if(world.time < 18000) // Ten minute grace period to let the game get going without lolmetagaming. -- TLE
		user << "Can not fueling transport. Please wait another [round((18000-world.time)/600)] minutes before trying again."
		return

	if(emergency_shuttle.direction == -1)
		user << "Transport may not be called while returning to Metropolis."
		return

	if(emergency_shuttle.online)
		user << "Transport is already and start fueling process."
		return
	...
	log_game("[key_name(user)] has start fueling transport.")
	message_admins("[key_name_admin(user)] has start fueling transport.", 1)
	captain_announce("The Transport fueling. [round(emergency_shuttle.timeleft()/60)] minutes left.")
...
/proc/init_shift_change(var/mob/user)
	...
	if(world.time < 54000) // 30 minute grace period to let the game get going
		user << "The transport is refueling. Please wait another [round((54000-world.time)/600)] minutes before trying again."//may need to change "/600"
		return
	...
	captain_announce("A crew transfer has been initiated. The Transport is not ready. Process will be completed in [round(emergency_shuttle.timeleft()/60)] minutes.")
...
/obj/machinery/computer/communications/Del()
	...
	log_game("All the AIs, comm consoles and boards are destroyed. Transport fueling.")
	message_admins("All the AIs, comm consoles and boards are destroyed. Transport fueling.", 1)
	captain_announce("The Transport fueling. Process will be completed in [round(emergency_shuttle.timeleft()/60)] minutes.")
	world << sound('shuttlecalled.ogg')
