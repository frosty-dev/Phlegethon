...
/obj/machinery/computer/arrival_shuttle/attack_hand(user as mob)
	...
	var/dat
	dat = text("<center>Escape Transport:<br> <b><A href='?src=\ref[src];move=[1]'>Send</A></b></center>")
...
/obj/machinery/computer/arrival_shuttle/Topic(href, href_list)
	...
	if(href_list["move"])
		if (!arrival_shuttle_moving)
			usr << "\blue Transport recieved message and start fueling."
			move_arrival_shuttle()
		else
			usr << "\blue Transport is already fueling."
