...
/obj/mecha/proc/mechturn(direction)
	dir = direction
	playsound(src,'carturn.ogg',40,1)
	return 1

/obj/mecha/proc/mechstep(direction)
	var/result = step(src,direction)
	if(result)
		playsound(src,'carstep.ogg',40,1)
	return result
...
/obj/mecha/verb/toggle_lights()
	...
	lights = !lights
	if(lights)
		ul_SetLuminosity(LuminosityRed + lights_power, LuminosityGreen + lights_power, LuminosityBlue + lights_power)
	else
		ul_SetLuminosity(LuminosityRed - lights_power, LuminosityGreen - lights_power, LuminosityBlue - lights_power)
...
//	usr << "You start climbing into [src.name]"
	...
	if(enter_after(40,usr))
		...
		usr << "You stop entering the vehicle."
	return
...
/obj/mecha/proc/moved_inside(var/mob/living/carbon/human/H as mob)
	if(H && H.client && H in range(1))
		...
		*/
		...
		dir = dir_in
		playsound(src,'carcharge.ogg',40,1)
		return 1
	else
		return 0
