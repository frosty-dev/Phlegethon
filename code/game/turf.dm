			...
			if(istype(M:shoes, /obj/item/clothing/shoes/clown_shoes))
				...
				if(M.m_intent == "run")
					if(M.footstep >= 2)
						...
						M.footstep++
					if(M.footstep == 0)
						playsound(src, "dirtrun", 40, 1) // this will get annoying very fast.
...
/turf/proc/ReplaceWithSpace()  //phleg turf
	var/old_dir = dir
	var/turf/space/S = new /turf/simulated/floor/plating( locate(src.x, src.y, src.z) )
	S.dir = old_dir
...
/turf/simulated/floor/plating_light
	name = "plating"
	icon_plating = "plating-light"
	icon_state = "plating-light"
	floor_tile = null
	intact = 0
