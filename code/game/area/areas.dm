...
//			A << sound('shipambience.ogg', repeat = 1, wait = 0, volume = 35, channel = 2)
		...
		switch(src.name)
			...
			if ("Space") sound = pick('ambispace.ogg','ambispace2.ogg',)
			...
			if ("AI Upload Foyer") sound = pick('ambimalf.ogg',"ambimalf2.ogg","ambimalf3.ogg")
			if ("AI Upload Chamber") sound = pick('ambimalf.ogg')
			if ("Mine")
				sound = pick('ambimine.ogg',"ambimine2.ogg")
				musVolume = 25
			else
...
/* fucking luminosity do not want work with area. Sucks!
/area/proc/daypassing() //phleg

//	var/showstart = pick(0, 1440, 2880, 4320)
//	var/areatime = world.time+1000// + showstart

//	for(var/area/street/AR in src.related)
		//src.updateicon()
//		for(var/turf/simulated/floor/F in AR)
//			F.ul_SetLuminosity(AR.streetluminosity)

	if(src.name == "Street")
		for(istype(/turf/simulated/floor in src.contents))
			var/areatime = world.time+1000
			switch(areatime)
		...
		//			streetluminosity = 0
				...
				if(1440 to 1680)	//0600-7000
					ul_SetLuminosity (1,1,1)
		//			streetluminosity = 1

				if(1680 to 1920)	//0700-0800
					ul_SetLuminosity (2,2,2)
		//			streetluminosity = 2

				if(1920 to 2160)	//0800-0900
					ul_SetLuminosity (3,3,3)
		//			streetluminosity = 3

				if(2160 to 2400)	//0900-1000
					ul_SetLuminosity (4,4,4)
		//			streetluminosity = 4
