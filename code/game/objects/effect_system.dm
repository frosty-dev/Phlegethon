...
/obj/effect/effect/dust_smoke
	name = "dust"
	icon_state = "dustsmoke"
	opacity = 1
	anchored = 0.0
	mouse_opacity = 0
	var/amount = 6.0
...
/obj/effect/effect/dust_smoke/New()
	..()
	spawn (100)
		del(src)
	return

/obj/effect/effect/dust_smoke/Move()
	..()
	return

/datum/effect/effect/system/dust_smoke_spread
	var/total_smoke = 0 // To stop it being spammed and lagging!
	var/direction
	...
	start()
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				if(holder)
					src.location = get_turf(holder)
				var/obj/effect/effect/dust_smoke/smoke = new /obj/effect/effect/dust_smoke(src.location)
				src.total_smoke++
				var/direction = src.direction
				if(!direction)
					if(src.cardinals)
...
/obj/effect/effect/frag_gas
	name = "nanobots shrapnel cloud"
	icon_state = "shrapnel"
	opacity = 0
	anchored = 0.0
	mouse_opacity = 0
...
/obj/effect/effect/frag_gas/New()
	..()
	spawn (100)
		del(src)
	return

/obj/effect/effect/frag_gas/Move()
	..()
	for(var/mob/living/carbon/human/R in get_turf(src))
		if (istype(R.wear_suit, /obj/item/clothing/suit/armor/captain))
		else
			R.burn_skin(0.75)
			if (R.coughedtime != 1)
				var/damage = roll("8d10")
				R.ear_damage += rand(3, 6)
				R.adjustBruteLoss(damage)
				R.coughedtime = 1
				R.emote("gasp")
				spawn (10)
					R.coughedtime = 0
...
/obj/effect/effect/frag_gas/HasEntered(mob/living/carbon/human/R as mob )
	..()
	if (istype(R, /mob/living/carbon/human))
		if (istype(R.wear_suit, /obj/item/clothing/suit/armor/captain))
			return
...
/datum/effect/effect/system/frag_gas_spread
	var/total_smoke = 0 // To stop it being spammed and lagging!
	var/direction
	...
	start()
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				if(holder)
					src.location = get_turf(holder)
				var/obj/effect/effect/frag_gas/smoke = new /obj/effect/effect/frag_gas(src.location)
				src.total_smoke++
				var/direction = src.direction
				if(!direction)
					if(src.cardinals)
...
/obj/effect/effect/fire_gas/New()
	..()
	src.ul_SetLuminosity(3,1,0)
	spawn(150+rand(30,80)) //10,50
		del(src)
	return

/obj/effect/effect/fire_gas/Move()
	..()
	for(var/mob/living/carbon/human/R in get_turf(src))
		R.burn_skin(1)
		var/damage = roll("1d10")
		R.ear_damage += rand(3, 6)
		R.adjustFireLoss(damage)
		R.coughedtime = 1
		R.emote("gasp")
		if(prob(60))
			R.burninglit(R)
...
/obj/effect/effect/fire_gas/HasEntered(mob/living/carbon/human/R as mob )
	..()
	if (istype(R, /mob/living/carbon/human))
		R.burn_skin(1)
		var/damage = roll("1d10")
...
/datum/effect/effect/system/fire_gas_spread
	var/total_smoke = 0 // To stop it being spammed and lagging!
	var/direction
	...
	start()
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				if(holder)
					src.location = get_turf(holder)
				var/obj/effect/effect/fire_gas/smoke = new /obj/effect/effect/fire_gas(src.location)
				src.total_smoke++
				var/direction = src.direction
				if(!direction)
					if(src.cardinals)
