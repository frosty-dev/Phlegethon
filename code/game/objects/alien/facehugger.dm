...
/obj/alien/facehugger
	...
	bullet_act(flag, A as obj)
		switch(flag)
			if (/obj/item/projectile/bullet)
				health -= 20
			if (/obj/item/projectile/bullet/weakbullet)
				health -= 4
			if (/obj/item/projectile/beam)
				health -= 10
			if (/obj/item/projectile/beam/pulse)
				health -= 35
		healthcheck()
