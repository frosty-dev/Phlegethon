...
/obj/item/weapon/fragmk
	desc = "It is set to detonate in 3 seconds. High powered fragmentation grenade"
	name = "Frag Grenade MK2A7"
	icon = 'grenade.dmi'
	icon_state = "fragmk"
	w_class = 2.0
	item_state = "flashbang"
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		if (isscrewdriver(W))
			switch(src.det_time)
				if ("1")
					src.det_time = 30
					user.show_message("\blue You set the frag grenade for 3 second detonation time.")
					src.desc = "It is set to detonate in 3 seconds."
				if ("30")
					src.det_time = 100
					user.show_message("\blue You set the frag grenade for 10 second detonation time.")
					src.desc = "It is set to detonate in 10 seconds."
				if ("100")
					src.det_time = 1
					user.show_message("\blue You set the frag grenade for instant detonation.")
					src.desc = "It is set to detonate instantly."
			src.add_fingerprint(user)
		..()
		return
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (istype(target, /obj/item/weapon/gun/grenadelauncher)) return ..()
		if((user.equipped() == src)&&(!active))
			user << "\red You prime the frag grenade! [det_time/10] seconds!"

			log_attack("<font color='red'>[user.name] ([user.ckey]) primed a frag grenade.</font>")
			log_admin("ATTACK: [user] ([user.ckey]) primed a frag grenade.")
			message_admins("ATTACK: [user] ([user.ckey])(<a href=\"byond://?src=%admin_ref%;teleto=\ref[user]\">Jump</a>) primed a frag grenade.")

			src.active = 1
			src.icon_state = "fragmk1"
			playsound(src.loc, 'armfrag.ogg', 75, 1, -3)
			spawn(src.det_time)
				prime()
				return
			user.dir = get_dir(user, target)
	...
	bang(var/turf/T , var/mob/living/carbon/M)						// Added a new proc called 'bang' that takes a location and a person to be banged.
		M << "\red <B>BANG</B>"
		playsound(src.loc, 'fragplosive.ogg', 25, 1)
		new/obj/effect/effect/fragexp(src.loc)
//Checking for protections
		var/ear_safety = 0
		sleep(3)
		explosion(loc, -1, 0, 2)
...
//Now applying sound
		if((get_dist(M, T) <= 2 || src.loc == M.loc || src.loc == M))
			M.Stun(10)
			M.Weaken(3)
			var/damage = roll("4d10")
			M.apply_damage(damage, BRUTE, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BRUTE, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BRUTE, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BURN, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BURN, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BURN, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))

			var/datum/organ/external/EO1 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			EO1.btrauma += rand(1,4)

			var/datum/organ/external/EO2 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			EO2.btrauma += rand(1,4)

			var/datum/organ/external/EO3 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			...
			var/datum/organ/external/EO1 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			EO1.btrauma += rand(1,3)

			var/datum/organ/external/EO2 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			EO2.btrauma += rand(1,3)

		else if(get_dist(M, T) <= 5)
			var/damage = roll("2d10")
			M.apply_damage(damage, BRUTE, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BRUTE, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))
			M.apply_damage(damage, BRUTE, pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm"))

			var/datum/organ/external/EO1 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			EO1.btrauma += rand(1,2)

			var/datum/organ/external/EO2 = M.organs[pick("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")]
			EO2.btrauma += rand(1,2)
