...
/obj/effect/biomass
	...
	var/maxDistance = 5 //the maximum length of a thread
	var/newSpreadDistance = 3 //the length of a thread at which new ones are created
...
/obj/effect/biomass/proc/NewSpread(maxDistance = 5)
	set background = 1
	for(var/turf/T in orange(1,src))
		if(!IsValidBiomassLoc(T,src))
			continue
